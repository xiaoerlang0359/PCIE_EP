...
// /___/   /\     Filename : X_SIM_CONFIG_V5.v
// \   \  /  \    Timestamp : 
//  \___\/\___\
//
// Revision:
...
//  reg rst_en = 1;
  ...
  initial begin
    if (DEVICE_ID == 32'h0) begin
      $display("Attribute Error : The attribute DEVICE_ID on  X_SIM_CONFIG_V5 instance %m is not set.");
      $finish;
     end
   end
    ...
    if (prog_b_t==0)
       ...
       if (prog_b_t)  begin
          ...
          if (m_in != 3'b110) begin
             $display("Error: input M is %h. Only Slave SelectMAP mode M=110 supported on X_SIM_CONFIG_V5 instance %m.", m_in);
          end
       end     
       else if ($time != 0)
       $display("Error: PROGB is not high when INITB goes high on X_SIM_CONFIG_V5 instance %m at time %t.", $time);
    end

  always @(m_in)
    if (mode_sample_flag == 1 && persist_en == 1)
       $display("Error : Mode pine M[2:0] changed after rising edge of INITB on X_SIM_CONFIG_V5 instance %m at time %t.", $time);
  ...
  always @(posedge prog_b_in or negedge prog_b_in)
    if (prog_b_in ==0) 
       ...
       if (prog_pulse_low < cfg_Tprog )
        $display("Error: Low time of PROGB is less than required minimum Tprogram time %d on X_SIM_CONFIG_V5 instance %m at time %t.", cfg_Tprog, $time);
    end
...
//                     if (tmp_byte == 8'hDD)
                     ...
                     if (tmp_byte == 8'h11) begin
                         ...
                         buswidth <= 2'b00;
                         buswid_flag_init <= 0;
                         $display("Error : BUS Width Auto Dection did not find 0x11 or 0x22 or 0x44 on D[7:0] followed 0xBB on X_SIM_CONFIG_V5 instance %m at time %t.", $time);
                         $finish;
                     end
                 end
            end
...
//              5'b01101 : cbc_reg <= pack_in_reg;
               5'b01100 : begin
                          ...
                          if (pack_in_reg[27:0] != DEVICE_ID[27:0]) begin
                             id_error_flag <= 1;
                             $display("Error : written value to IDCODE register is %h which does not match DEVICE ID %h on X_SIM_CONFIG_V5 instance %m at time %t.", pack_in_reg, DEVICE_ID, $time);
                          end 
                          else
                             id_error_flag <= 0;
                          end
...
//                $display("Warning :  only Type 1 Packet supported on X_SIM_CONFIG_V5 instance %m at time %t.", $time);
                cmd_rd_flag <= 0;
                cmd_wr_flag <= 0;
                conti_data_flag <= 0;
                conti_data_cnt <= 0;
    ...
    always @(posedge rdwr_b_in  or negedge rst_intl or
      ...
      if (rst_intl==0 || abort_flag_rst==1 || csi_b_in == 1)
        ...
        if (abort_dis == 0 && csi_b_in == 0) begin
             if ($time != 0) begin
               abort_flag_wr <= 1;
               $display(" Warning : RDWRB changes when CSB low, which causes Configuration abort on X_SIM_CONFIG_V5 instance %m at time %t.", $time);
             end
        end
        else
           abort_flag_wr <= 0;
