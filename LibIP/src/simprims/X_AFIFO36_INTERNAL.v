// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/versclibs/data/rainier/X_AFIFO36_INTERNAL.v,v 1.22 2010/12/09 01:02:12 wloo Exp $
///////////////////////////////////////////////////////////////////////////////
...
module X_AFIFO36_INTERNAL (ALMOSTEMPTY, ALMOSTFULL, DBITERR, DO, DOP, ECCPARITY, EMPTY, FULL, RDCOUNT, RDERR, SBITERR, WRCOUNT, WRERR,
    ...
    integer roundup_int_period_rdclk_wrclk;
...
// xilinx_internal_parameter off
	...
	if (gsr_in == 1'b1 || rst_in == 1'b1) begin
	    ...
	    assign wr1_flag = 0;
	    assign awr_flag = 0;
	    assign period_rdclk = 0;
	    assign rise_rdclk = 0;
	    assign period_wrclk = 0;
	    assign rise_wrclk = 0;
	    assign aempty_flag = 0;
	    assign afull_flag = 0;
	end
	    ...
	    deassign wr1_flag;
	    deassign awr_flag;	
    	    deassign period_rdclk;
	    deassign rise_rdclk;
	    deassign period_wrclk;
	    deassign rise_wrclk;
	    deassign aempty_flag;
	    deassign afull_flag;
	end
	...
	if (rst_tmp1 == 0 && rst_tmp2 == 1) begin
		...
		$display("DRC Error : At time %t,  RST has to be held high for more than three RDCLK cycles on AFIFO36_INTERNAL instance %m.", $stime);
		rst_rdclk_flag = 1;
	    end

            if ((rst_wrckreg[2] & rst_wrckreg[1] & rst_wrckreg[0]) == 0) begin
		$display("DRC Error : At time %t,  RST has to be held high for more than three WRCLK cycles on AFIFO36_INTERNAL instance %m.", $stime);
		rst_wrclk_flag = 1;
	    end
	...
	if (rst_in == 1'b0) begin
		...
		if (ALMOST_EMPTY_OFFSET <= (4 * roundup_int_period_rdclk_wrclk)) begin

		    $display("DRC Error : The attribute ALMOST_EMPTY_OFFSET on X_AFIFO36_INTERNAL instance %m is set to %d. It must be set to a value greater than (4 * roundup (WRCLK frequency / RDCLK frequency)) when X_AFIFO36_INTERNAL is configured in FIRST_WORD_FALL_THROUGH mode.", ALMOST_EMPTY_OFFSET);
		    $finish;
