// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/versclibs/data/trilogy/X_SPI_ACCESS.v,v 1.20 2011/03/24 16:48:51 robh Exp $
///////////////////////////////////////////////////////////////////////////////
...
integer i;
integer display_warning;
begin
    display_warning = 0;
    if (buf_type == 1)
        for (i=0 ; i < PAGESIZE; i = i+1)
            begin
            memory[low+i] = buffer1[i] & memory[low+i] ;
            if (buffer1[i] != memory[low+i]) display_warning = 1;
            end
    else if (buf_type == 2)
         for (i=0 ; i < PAGESIZE; i = i+1)
            begin
            memory[low+i] = buffer2[i] & memory[low+i] ;
            if (buffer2[i] != memory[low+i]) display_warning = 1;
            end
    if (display_warning == 1) begin
       $display ("DRC Warning : In %s at: %t, attempt to program bits to 1 on page %d without an erase. Overwriting 0 with 1 requires a page erase first.", name, $time, page);
    end
...
/******* Buffer To Main Memory Page Prog With/Without Built In Erase *******/
...
begin : BTMMPP_
    ...
    // which can be obtained correctly
    @ (posedge CSB_in);
//    writing to a non erased page could be ok, disable annoying warning.
//    but warn during write if writing 1's on top of 0's - not allowed.
//    if (page_status[page] != 1'b0 && erase_flag ==0)
//    $display ("DRC Warning : In %s trying to write into Page %d which is not erased", name, page);
    RDYBSY_reg = 1'b0; // device is busy
    status[7] = 1'b0;
    if (erase_flag ==1) begin
