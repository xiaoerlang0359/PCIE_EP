...
//    07/19/11 - CR 607362 -- fixed simprim DELAY_D to make it transport
//    09/21/11 - CR 623071 - Updated REFCLK_FREQUENCY check message
//    12/15/11 - CR 615802 - Changed Average tap vaules based on REFCLK_FREQUENCY
// End Revision
...
module X_IODELAYE1 (CNTVALUEOUT, DATAOUT, C, CE, CINVCTRL, CLKIN, CNTVALUEIN, DATAIN, IDATAIN, INC, ODATAIN, RST, T);
    ...
    parameter integer SIM_ODELAY_D = 0;
    localparam DELAY_D  = (IDELAY_TYPE == "FIXED") ? 0 : SIM_DELAY_D;
    localparam ODELAY_D = (ODELAY_TYPE == "FIXED") ? 0 : SIM_ODELAY_D;
...
//    02/02/09 - Fixed CNTVALUEOUT for when in DEFAULT/FIXED mode because of simprim. 
    ...
    initial begin
        ...
        if (REFCLK_FREQUENCY < MIN_REFCLK_FREQUENCYL || REFCLK_FREQUENCY > MAX_REFCLK_FREQUENCYH) begin
            $display("Attribute Syntax Error : The attribute REFCLK_FREQUENCY on X_IODELAYE1 instance %m is set to %f.  Legal values for this attribute are either between 190.0 and 210.0, or between 290.0 and 310.0", REFCLK_FREQUENCY);
        ...
        //-------- CALC_TAPDELAY check
// CR 615802 -- calculate the tap value based on REFCLK_FREQUENCY
...
//*********************************************************
    ...
    always @(datain_in or idatain_in, odatain_in, t_in, clkin_in ) begin
        ...
        case (DELAY_SRC)
            ...
            "I" : begin
                         data_mux <= #DELAY_D idatain_in;
                  end
            "O" : begin
                         data_mux <= #ODELAY_D odatain_in;
                  end
            "IO" : begin
                         if(t_in == 1'b1) 
                             data_mux <= #DELAY_D idatain_in; 
