// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/versclibs/data/stan/X_RAMB8BWER.v,v 1.22 2011/08/04 18:28:40 wloo Exp $
///////////////////////////////////////////////////////////////////////////////
...
//    08/04/11 - Changed RST_PRIORITY default to CE (CR 618583).
// End Revision
...
module X_RAMB8BWER (DOADO, DOBDO, DOPADOP, DOPBDOP, 
    ...
    wire [12:0] ox_addra_reconstruct, ox_addrb_reconstruct;
    reg [12:0] 	ox_addra_reconstruct_reg, ox_addrb_reconstruct_reg;
    ...
    assign ox_addra_reconstruct[12:0] = (WRITE_MODE_A == "READ_FIRST" || WRITE_MODE_B == "READ_FIRST") ? 
					({addrawraddr_in[12:6],1'b0,addrawraddr_in[4],4'b0}) : addrawraddr_in[12:0];

    assign ox_addrb_reconstruct[12:0] = (WRITE_MODE_A == "READ_FIRST" || WRITE_MODE_B == "READ_FIRST") ?
	...
	case (RAM_MODE)
	    ...
	    "SDP" : begin
			    ...
			    $display("DRC Error : Both attributes WRITE_MODE_A and WRITE_MODE_B must be set to READ_FIRST or both attributes must be set to WRITE_FIRST when RAM_MODE = SDP on X_RAMB8BWER instance %m.");
			    finish_error = 1;
			end
...
/************************************* Collision starts *****************************************/
	  ...
	  if (SIM_COLLISION_CHECK != "NONE") begin
		...
		if (viol_time != 0) begin
			...
			if (addrawraddr_in[12:col_addr_lsb] === addrbrdaddr_in[12:col_addr_lsb]) begin
			...
			else if ((wr_mode_a == 2'b01 || wr_mode_b == 2'b01) && (ox_addra_reconstruct[12:col_addr_lsb] === ox_addrb_reconstruct[12:col_addr_lsb])) begin
			    ...
			    if (wr_mode_b != 2'b01)
				task_col_rd_ram_b (2'b11, weawel_int, webweu_int, addrbrdaddr_in, dobdo_buf, dopbdop_buf);
			end // if ((wr_mode_a == 2'b01 || wr_mode_b == 2'b01) && (ox_addra_reconstruct[12:col_addr_lsb] === ox_addrb_reconstruct[12:col_addr_lsb]))
			else
				viol_time = 0;
			
		    end
		    else if (rising_clkawrclk && !rising_clkbrdclk) begin
			if (addrawraddr_in[12:col_addr_lsb] === addrbrdaddr_reg[12:col_addr_lsb]) begin
			...
			else if ((wr_mode_a == 2'b01 || wr_mode_b == 2'b01) && (ox_addra_reconstruct[12:col_addr_lsb] === ox_addrb_reconstruct_reg[12:col_addr_lsb])) begin
			...
			end // if ((wr_mode_a == 2'b01 || wr_mode_b == 2'b01) && (ox_addra_reconstruct[12:col_addr_lsb] === ox_addrb_reconstruct_reg[12:col_addr_lsb]))
			else
			    viol_time = 0;
			
		    end
		    else if (!rising_clkawrclk && rising_clkbrdclk) begin
			if (addrawraddr_reg[12:col_addr_lsb] === addrbrdaddr_in[12:col_addr_lsb]) begin
			...
			else if  ((wr_mode_a == 2'b01 || wr_mode_b == 2'b01) && (ox_addra_reconstruct_reg[12:col_addr_lsb] === ox_addrb_reconstruct[12:col_addr_lsb])) begin
			...
			end // if ((wr_mode_a == 2'b01 || wr_mode_b == 2'b01) && (ox_addra_reconstruct_reg[12:col_addr_lsb] === ox_addrb_reconstruct[12:col_addr_lsb]))
			else
			    viol_time = 0;
...
/******************************************** task and function **************************************/
	...
	begin
		...
		if (weawel_tmp === 1'b1 && webweu_tmp === 1'b1 && col_wr_wr_msg == 1) begin
		...
		end // if (weawel_tmp === 1'b1 && webweu_tmp === 1'b1 && col_wr_wr_msg == 1)
			...
			if ((wr_mode_a == 2'b01 || wr_mode_b == 2'b01) && (!(chk_col_same_clk == 1)))
