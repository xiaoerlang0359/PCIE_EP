// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/versclibs/data/blanc/X_FIFO18E1.v,v 1.38.58.3 2013/08/01 23:41:17 wloo Exp $
///////////////////////////////////////////////////////////////////////////////
...
module X_FIFO18E1 (ALMOSTEMPTY, ALMOSTFULL, DO, DOP, EMPTY, FULL, RDCOUNT, RDERR, WRCOUNT, WRERR,
    ...
    wire rdclk_in, RDCLK_dly;
    wire rden_in, RDEN_dly;
    wire regce_in, REGCE_dly;
    wire rst_in, RST_dly;
    wire rstreg_in, RSTREG_dly;
    wire wrclk_in, WRCLK_dly;
    wire wren_in, WREN_dly;
    
    assign di_in = DI_dly;
    assign dip_in = DIP_dly;
    assign rdclk_in = RDCLK_dly;
    assign rden_in = RDEN_dly;
    assign regce_in = REGCE_dly;
    assign rst_in = RST_dly;
    assign rstreg_in = RSTREG_dly;
    assign wrclk_in = WRCLK_dly;
    assign wren_in = WREN_dly;
...
// xilinx_internal_parameter off
	...
	case (DATA_WIDTH)
	   ...
	   default :
		begin
		    $display("Attribute Syntax Error : The attribute DATA_WIDTH on X_FIFO18E1 instance %m is set to %d.  Legal values for this attribute are 4, 9, 18, 36 or 72.", DATA_WIDTH);
		    finish_error = 1;
		end
	endcase
	...
	case (EN_SYN)
	    ...
	    "TRUE"  : sync = 1;
	    default : begin
		          $display("Attribute Syntax Error : The attribute EN_SYN on X_FIFO18E1 instance %m is set to %s.  Legal values for this attribute are TRUE or FALSE.", EN_SYN);
		          finish_error = 1;
	              end
	endcase // case(EN_SYN)
	...
	case (FIRST_WORD_FALL_THROUGH)
	    ...
	    default : begin
		$display("Attribute Syntax Error : The attribute FIRST_WORD_FALL_THROUGH on X_FIFO18E1 instance %m is set to %s.  Legal values for this attribute are TRUE or FALSE.", FIRST_WORD_FALL_THROUGH);
		finish_error = 1;
	      end
	endcase
	...
	if (fwft == 1'b1 && EN_SYN == "TRUE") begin
	    $display("DRC Error : First word fall through is not supported in synchronous mode on X_FIFO18E1 instance %m.");
	    finish_error = 1;
	end

	if (EN_SYN == "FALSE" && DO_REG == 0) begin
	    $display("DRC Error : DO_REG = 0 is invalid when EN_SYN is set to FALSE on X_FIFO18E1 instance %m.");
	    finish_error = 1;
	end
	...
	case (EN_ECC_WRITE)
	    ...
	    default : begin
	       	          $display("Attribute Syntax Error : The attribute EN_ECC_WRITE on X_FIFO18E1 instance %m is set to %s.  Legal values for this attribute are TRUE or FALSE.", EN_ECC_WRITE);
		          finish_error = 1;
		      end
	endcase
	...
	case (EN_ECC_READ)
	    ...
	    default : begin
	       	          $display("Attribute Syntax Error : The attribute EN_ECC_READ on X_FIFO18E1 instance %m is set to %s.  Legal values for this attribute are TRUE or FALSE.", EN_ECC_READ);
		          finish_error = 1;
		      end
	endcase
	...
	if ((EN_ECC_READ == "TRUE" || EN_ECC_WRITE == "TRUE") && DATA_WIDTH != 72) begin
	    $display("DRC Error : The attribute DATA_WIDTH must be set to 72 when X_FIFO18E1 is configured in the ECC mode.");
	    finish_error = 1;
	end
		...
		if (period_rdclk == period_wrclk) begin
			...
			if (SIM_DEVICE == "7SERIES") begin
				...
				if ((ALMOST_EMPTY_OFFSET < 5) || (ALMOST_EMPTY_OFFSET > addr_limit - 6)) begin
				    $display("Attribute Syntax Error : The attribute ALMOST_EMPTY_OFFSET on X_FIFO18E1 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_EMPTY_OFFSET, 5, addr_limit - 6);
				    $finish;
				end
				
				if ((ALMOST_FULL_OFFSET < 4) || (ALMOST_FULL_OFFSET > addr_limit - 7)) begin
				    $display("Attribute Syntax Error : The attribute ALMOST_FULL_OFFSET on X_FIFO18E1 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_FULL_OFFSET, 4, addr_limit - 7);
				    $finish;
				end
				...
				if ((ALMOST_EMPTY_OFFSET < 6) || (ALMOST_EMPTY_OFFSET > addr_limit - 5)) begin
				    $display("Attribute Syntax Error : The attribute ALMOST_EMPTY_OFFSET on X_FIFO18E1 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_EMPTY_OFFSET, 6, addr_limit - 5);
				    $finish;
				end
				...
				if ((ALMOST_EMPTY_OFFSET < 5) || (ALMOST_EMPTY_OFFSET > addr_limit - 5)) begin
				    $display("Attribute Syntax Error : The attribute ALMOST_EMPTY_OFFSET on X_FIFO18E1 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_EMPTY_OFFSET, 5, addr_limit - 5);
				    $finish;
			    end
				
			    if ((ALMOST_FULL_OFFSET < 4) || (ALMOST_FULL_OFFSET > addr_limit - 5)) begin
				$display("Attribute Syntax Error : The attribute ALMOST_FULL_OFFSET on X_FIFO18E1 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_FULL_OFFSET, 4, addr_limit - 5);
				$finish;
				...
				if ((ALMOST_EMPTY_OFFSET < 6) || (ALMOST_EMPTY_OFFSET > addr_limit - 4)) begin
				    $display("Attribute Syntax Error : The attribute ALMOST_EMPTY_OFFSET on X_FIFO18E1 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_EMPTY_OFFSET, 6, addr_limit - 4);
				    $finish;
				end
			...
			if ((fwft == 1'b0) && ((ALMOST_EMPTY_OFFSET < 1) || (ALMOST_EMPTY_OFFSET > addr_limit - 2))) begin
			    $display("Attribute Syntax Error : The attribute ALMOST_EMPTY_OFFSET on X_FIFO18E1 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_EMPTY_OFFSET, 1, addr_limit - 2);
			    $finish;
			end
			
			if ((fwft == 1'b0) && ((ALMOST_FULL_OFFSET < 1) || (ALMOST_FULL_OFFSET > addr_limit - 2))) begin
			    $display("Attribute Syntax Error : The attribute ALMOST_FULL_OFFSET on X_FIFO18E1 instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_FULL_OFFSET, 1, addr_limit - 2);
			    $finish;
			end
			...
			if (ALMOST_FULL_OFFSET > (addr_limit - (s7_roundup_int_period_rdclk_wrclk + 6))) begin
			    ...
			    $display("DRC Error : The attribute ALMOST_FULL_OFFSET on X_FIFO18E1 instance %m is set to %d. It must be set to a value smaller than (FIFO_DEPTH - ((roundup(4 * (WRCLK frequency / RDCLK frequency))) + 6)) when X_FIFO18E1 has different frequencies for RDCLK and WRCLK.", ALMOST_FULL_OFFSET);
			    $finish;
			...
			if (ALMOST_FULL_OFFSET > (addr_limit - ((3 * roundup_int_period_wrclk_rdclk) + 3))) begin
			    ...
			    $display("DRC Error : The attribute ALMOST_FULL_OFFSET on X_FIFO18E1 instance %m is set to %d. It must be set to a value smaller than (FIFO_DEPTH - ((3 * roundup (RDCLK frequency / WRCLK frequency)) + 3)) when X_FIFO18E1 has different frequencies for RDCLK and WRCLK.", ALMOST_FULL_OFFSET);
			    $finish;
			...
			if (ALMOST_EMPTY_OFFSET > (addr_limit - ((3 * roundup_int_period_rdclk_wrclk) + 3))) begin
			    ...
			    $display("DRC Error : The attribute ALMOST_EMPTY_OFFSET on X_FIFO18E1 instance %m is set to %d. It must be set to a value smaller than (FIFO_DEPTH - ((3 * roundup (WRCLK frequency / RDCLK frequency)) + 3)) when X_FIFO18E1 has different frequencies for RDCLK and WRCLK.", ALMOST_EMPTY_OFFSET);
			    $finish;
  ...
  generate
	...
	if ((injectsbiterr_in === 1) && !(en_ecc_write_int == 1 || en_ecc_read_int == 1)) 
	    $display("DRC Warning : INJECTSBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on X_FIFO18E1 instance %m.");
	
	if ((injectdbiterr_in === 1) && !(en_ecc_write_int == 1 || en_ecc_read_int == 1)) 
	    $display("DRC Warning : INJECTDBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on X_FIFO18E1 instance %m.");
...
endgenerate
	...
	if (sync == 1)
		...
		default : begin
	                      $display("Attribute Syntax Error : The attribute DO_REG on X_FIFO18E1 instance %m is set to %2d.  Legal values for this attribute are 0 or 1.", DO_REG);
	                      $finish;
	                  end
	    endcase
...
// end of X_FF18_INTERNAL_VLOG - Note: Not an user primitvie
