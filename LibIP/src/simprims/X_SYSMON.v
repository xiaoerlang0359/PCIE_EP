...
//  `ifndef XIL_TIMING
    ...
    buf b_busy (BUSY, busy_out);
    buf b_drdy (DRDY, drdy_out);
    buf b_eoc (EOC, eoc_out);
    buf b_eos (EOS, eos_out);
    buf b_ot (OT, ot_out);
    buf b_do[15:0] (DO, do_out);
    buf b_channel[4:0] (CHANNEL, channel_out);
    buf b_alarm[2:0] (ALM, alarm_out);

    buf b_convst (convst_raw_in, CONVST);
    buf b_convstclk (convstclk_in, CONVSTCLK);
    buf b_dclk (dclk_in, DCLK_dly);
    buf b_den (den_in, DEN_dly);
    buf b_rst (rst_input, RESET);
    buf b_dwe (dwe_in, DWE_dly);
    buf b_di[15:0] (di_in, DI_dly);
    buf b_daddr[6:0] (daddr_in, DADDR_dly);
...
//  `endif  `ifdef XIL_TIMING
    ...
    initial begin
        ...
        if ((init41h_tmp[13:12]==2'b11) && (init40h_tmp[8]==1) && (init40h_tmp[4:0] != 5'b00011) && (init40h_tmp[4:0] < 5'b10000))
             $display(" Attribute Syntax warning : The attribute INIT_40 on X_SYSMON instance %m is set to %x.  Bit[8] of this attribute must be set to 0. Long acquistion mode is only allowed for external channels", INIT_40);

        if ((init41h_tmp[13:12]!=2'b11) && (init4eh_tmp[10:0]!=11'b0) && (init4eh_tmp[15:12]!=4'b0))
             $display(" Attribute Syntax warning : The attribute INIT_4E on X_SYSMON instance %m is set to %x.  Bit[15:12] and bit[10:0] of this attribute must be set to 0. Long acquistion mode is only allowed for external channels", INIT_4E);

        if ((init41h_tmp[13:12]==2'b11) && (init40h_tmp[9]==1) && (init40h_tmp[4:0] != 5'b00011) && (init40h_tmp[4:0] < 5'b10000))
             $display(" Attribute Syntax warning : The attribute INIT_40 on X_SYSMON instance %m is set to %x.  Bit[9] of this attribute must be set to 0. Event mode timing can only be used with external channels, and only in single channel mode.", INIT_40);

        if ((init41h_tmp[13:12]==2'b11) && (init40h_tmp[13:12]!=2'b00) && (INIT_48 != 16'h0000) &&  (INIT_49 != 16'h0000))
             $display(" Attribute Syntax warning : INIT_48 and INIT_49 are %x and %x on X_SYSMON instance %m. Those attributes must be set to 0000h in single channel mode and averaging enabled.", INIT_48, INIT_49);

        if (init42h_tmp[1:0] != 2'b00) 
             $display(" Attribute Syntax Error : The attribute INIT_42 on X_SYSMON instance %m is set to %x.  Bit[1:0] of this attribute must be set to 0h.", INIT_42);

        if (SIM_DEVICE == "VIRTEX6") begin
          sim_device_int = 1;
          if (init42h_tmp[15:8] < 8'b00000010) begin 
             $display(" Attribute Syntax Error : The attribute INIT_42 on X_SYSMON instance %m is set to %x.  Bit[15:8] of this attribute is the ADC Clock divider and must be equal or greater than 2. ", INIT_42);
           $finish;
          end
        end
        else begin
          ...
          if (init42h_tmp[15:8] < 8'b00001000) begin 
             $display(" Attribute Syntax Error : The attribute INIT_42 on X_SYSMON instance %m is set to %x.  Bit[15:8] of this attribute is the ADC Clock divider and must be equal or greater than 8. ", INIT_42);
           $finish;
           end
        end
        
        if (INIT_43 != 16'h0) 
             $display(" Warning : The attribute INIT_43 on X_SYSMON instance %m is set to %x.  This must be set to 0000h.", INIT_43);
             
        if (INIT_44 != 16'h0) 
             $display(" Warning : The attribute INIT_44 on X_SYSMON instance %m is set to %x. This must be set to  0000h.", INIT_44);

        if (INIT_45 != 16'h0) 
             $display(" Warning : The attribute INIT_45 on X_SYSMON instance %m is set to %x.  This must be set to 0000h.", INIT_45);

        if (INIT_46 != 16'h0) 
             $display(" Warning : The attribute INIT_46 on X_SYSMON instance %m is set to %x.  This must be set to  0000h.", INIT_46);

        if (INIT_47 != 16'h0) 
             $display(" Warning : The attribute INIT_47 on X_SYSMON instance %m is set to %x.  This must be set to 0000h.", INIT_47);
...
// read input file
	...
	if  (fd == 0)
        begin
          $display(" *** Warning: The analog data file %s for X_SYSMON instance %m was not found. Use the SIM_MONITOR_FILE parameter to specify the analog data file name or use the default name: design.txt.\n", SIM_MONITOR_FILE);
          sim_file_flag = 1;
        end
...
//             if(char_2==`EOFile) 
		...
		for (m = 0; m < num_arg; m = m +1) begin
			...
			default : begin
	              $display("Analog Data File Error : The channel name %s is invalid in the input file for X_SYSMON instance %m.", tmp_label);
                                      infile_format;
 			          end
		    endcase
		...
		if (time_index != -1) begin
			...
			$display("Analog Data File Error : Time value %f is invalid in the input file for X_SYSMON instance %m. Time value should increase.", time_out);
                        infile_format;
		    end
		    ...
		    $display("Analog Data File Error : No TIME label is found in the analog data file for X_SYSMON instance %m.");
                    infile_format;
		    $finish;
		end
    ...
    task infile_format;
    begin
    $display("\n***** X_SYSMON Simulation Analog Data File Format *****\n");
...
// Check if (Vp+Vn)/2 = 0.5 +/- 100 mv,  unipolar only
    ...
    begin
      if (acq_b_u == 0 && rst_in == 0 && ((acq_chan == 3) || (acq_chan >= 16 && acq_chan <= 31))) begin  
            ...
            $display("Input File Warning: The N input for external channel %x must be smaller than P input when in unipolar mode (P=%0.2f N=%0.2f) for X_SYSMON instance %m at %.3f ns.", acq_chan, chan_val_p_tmp, chan_val_n_tmp, $time/1000.0);
            if ( chan_val_n_tmp > 0.5 || chan_val_n_tmp < 0.0)
            $display("Input File Warning: The range of N input for external channel %x should be between 0V to 0.5V when in unipolar mode (N=%0.2f) for X_SYSMON instance %m at %.3f ns.", acq_chan, chan_val_n_tmp, $time/1000.0);
      end
    end
     ...
     if (gsr_in == 1) begin
        ...
        if (den_in == 1) begin
          if (drdy_out_tmp1 == 0) begin
             ...
             $display("Warning : input pin DEN on X_SYSMON instance %m at time %.3f ns can not continue set to high. Need wait DRDY high and then set DEN high again.", $time/1000.0);  
          end
        end
        else
           drdy_out_tmp1 <= 0;
        ...
        $display("Invalid Input Warning : The DADDR %x to X_SYSMON instance %m at time %.3f ns is accessing an undefined location. The data in this location is invalid.", daddr_in, $time/1000.0);
        end
        else begin
...
//                    ot_limit_reg <= 16'hCA00;
          ...
          if (sim_device_int == 1) begin
            if ( daddr_in == 7'h42 && (di_in[2:0] !=3'b000)) 
                $display(" Invalid Input Error : The DI bit[2:0] %x at DADDR %x on X_SYSMON instance %m at %.3f ns is invalid. These must be set to 000.", di_in[2:0], daddr_in, $time/1000.0);
          end
          else begin
            if ( daddr_in == 7'h42 && (di_in[1:0] !=2'b00)) 
                $display(" Invalid Input Error : The DI bit[1:0] %x at DADDR %x on X_SYSMON instance %m at %.3f ns is invalid. These must be set to 00.", di_in[1:0], daddr_in, $time/1000.0);
          end

          if (sim_device_int == 0) begin
            if (daddr_in == 7'h42 && (di_in[15:8] < 8'b00001000)) begin
             $display(" Invalid Input Error : The  DI bit[15:8] %x at DADDR %x on X_SYSMON instance %m  at %.3f ns is invalid. Bit[15:8] of Control Register 42h is the ADC Clock divider and must be equal or greater than 8. ", di_in[15:8], daddr_in, $time/1000.0);
               $finish;
             end
           end

          if ( daddr_in >= 7'h43 && daddr_in <= 7'h47 && (di_in[15:0] != 16'h0000)) 
		$display(" Invalid Input Error : The DI value %x at DADDR %x of X_SYSMON instance %m at %.3f ns is invalid. These must be set to 0000h.", di_in, daddr_in, $time/1000.0);

          if ((daddr_in == 7'h40) && (di_in[4:0] == 5'b00110  || di_in[4:0] == 5'b00111
                         || (di_in[4:0] >= 5'b01010 && di_in[4:0] <= 5'b01111))) 
		$display("Invalid Input Warning : The DI bit4:0] at address DADDR %x to X_SYSMON instance %m at %.3f ns is %h, which is invalid analog channel.", daddr_in, $time/1000.0, di_in[4:0]);

          if (daddr_in == 7'h40) begin
            if ((cfg_reg1[13:12]==2'b11) && (di_in[8]==1) && (di_in[4:0] != 5'b00011) && (di_in[4:0] < 5'b10000))
             $display(" Invalid Input warning : The DI value is %x at DADDR %x on X_SYSMON instance %m at %.3f ns.  Bit[8] of DI must be set to 0. Long acquistion mode is only allowed for external channels", di_in, daddr_in, $time/1000.0);

          if ((cfg_reg1[13:12]==2'b11) && (di_in[9]==1) && (di_in[4:0] != 5'b00011) && (di_in[4:0] < 5'b10000))
             $display(" Invalid Input warning : The DI value is %x at DADDR %x on X_SYSMON instance %m at %.3f ns.  Bit[9] of DI must be set to 0. Event mode timing can only be used with external channels", di_in, daddr_in, $time/1000.0);

          if ((cfg_reg1[13:12]==2'b11) && (di_in[13:12]!=2'b00) && (seq_chan_reg1 != 16'h0000) &&  (seq_chan_reg2 != 16'h0000))
             $display(" Invalid Input warning : The  Control Regiter 48h and 49h are %x and %x on X_SYSMON instance %m at %.3f ns. Those registers should be set to 0000h in single channel mode and averaging enabled.", seq_chan_reg1, seq_chan_reg2, $time/1000.0);

         end

         if (daddr_in == 7'h41 && en_data_flag == 1) begin
           if ((di_in[13:12]==2'b11) && (cfg_reg0[8]==1) && (cfg_reg0[4:0] != 5'b00011) && (cfg_reg0[4:0] < 5'b10000))
             $display(" Invalid Input warning : The  Control Regiter 40h value is %x on X_SYSMON instance %m at %.3f ns.  Bit[8] of Control Regiter 40h  must be set to 0. Long acquistion mode is only allowed for external channels", cfg_reg0, $time/1000.0);

           if ((di_in[13:12]==2'b11) && (cfg_reg0[9]==1) && (cfg_reg0[4:0] != 5'b00011) && (cfg_reg0[4:0] < 5'b10000))
             $display(" Invalid Input warning : The  Control Regiter 40h value is %x on X_SYSMON instance %m at %.3f ns.  Bit[9] of Control Regiter 40h  must be set to 0. Event mode timing can only be used with external channels", cfg_reg0, $time/1000.0);

           if ((di_in[13:12]!=2'b11) && (seq_acq_reg1[10:0]!=11'b0) && (seq_acq_reg1[15:12]!=4'b0))
             $display(" Invalid Input warning : The Control Regiter 4Eh value is %x on X_SYSMON instance %m at %.3f ns.  Bit[15:12] and bit[10:0] of this register must be set to 0. Long acquistion mode is only allowed for external channels", seq_acq_reg1, $time/1000.0);
...
//                acq_e_c = 0;
   ...
   always @(posedge conv_start or posedge rst_in ) begin
	if (rst_in == 1) begin
		...
		$display("Invalid Input Warning : The analog channel %x to X_SYSMON instance %m at %.3f ns is invalid.", acq_chan, $time/1000.0);
   ...
   always @(posedge adcclk or posedge rst_in)
	if (rst_in == 1) begin
		...
		if (next_state == CONV_STATE)
			...
			$display ("Warning: Acquisition time is not long enough for X_SYSMON instance %m at time %t.", $time);
...
//  `ifdef XIL_TIMING
        ...
        $setuphold (posedge DCLK, negedge DADDR[0], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[0]);
        $setuphold (posedge DCLK, negedge DADDR[1], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[1]);
        $setuphold (posedge DCLK, negedge DADDR[2], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[2]);
        $setuphold (posedge DCLK, negedge DADDR[3], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[3]);
        $setuphold (posedge DCLK, negedge DADDR[4], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[4]);
        $setuphold (posedge DCLK, negedge DADDR[5], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[5]);
        $setuphold (posedge DCLK, negedge DADDR[6], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[6]);
        $setuphold (posedge DCLK, negedge DEN, 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DEN_dly);
        $setuphold (posedge DCLK, negedge DI[0], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[0]);
        $setuphold (posedge DCLK, negedge DI[10], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[10]);
        $setuphold (posedge DCLK, negedge DI[11], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[11]);
        $setuphold (posedge DCLK, negedge DI[12], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[12]);
        $setuphold (posedge DCLK, negedge DI[13], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[13]);
        $setuphold (posedge DCLK, negedge DI[14], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[14]);
        $setuphold (posedge DCLK, negedge DI[15], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[15]);
        $setuphold (posedge DCLK, negedge DI[1], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[1]);
        $setuphold (posedge DCLK, negedge DI[2], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[2]);
        $setuphold (posedge DCLK, negedge DI[3], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[3]);
        $setuphold (posedge DCLK, negedge DI[4], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[4]);
        $setuphold (posedge DCLK, negedge DI[5], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[5]);
        $setuphold (posedge DCLK, negedge DI[6], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[6]);
        $setuphold (posedge DCLK, negedge DI[7], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[7]);
        $setuphold (posedge DCLK, negedge DI[8], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[8]);
        $setuphold (posedge DCLK, negedge DI[9], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[9]);
        $setuphold (posedge DCLK, negedge DWE, 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DWE_dly);
        $setuphold (posedge DCLK, posedge DADDR[0], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[0]);
        $setuphold (posedge DCLK, posedge DADDR[1], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[1]);
        $setuphold (posedge DCLK, posedge DADDR[2], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[2]);
        $setuphold (posedge DCLK, posedge DADDR[3], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[3]);
        $setuphold (posedge DCLK, posedge DADDR[4], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[4]);
        $setuphold (posedge DCLK, posedge DADDR[5], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[5]);
        $setuphold (posedge DCLK, posedge DADDR[6], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DADDR_dly[6]);
        $setuphold (posedge DCLK, posedge DEN, 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DEN_dly);
        $setuphold (posedge DCLK, posedge DI[0], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[0]);
        $setuphold (posedge DCLK, posedge DI[10], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[10]);
        $setuphold (posedge DCLK, posedge DI[11], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[11]);
        $setuphold (posedge DCLK, posedge DI[12], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[12]);
        $setuphold (posedge DCLK, posedge DI[13], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[13]);
        $setuphold (posedge DCLK, posedge DI[14], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[14]);
        $setuphold (posedge DCLK, posedge DI[15], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[15]);
        $setuphold (posedge DCLK, posedge DI[1], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[1]);
        $setuphold (posedge DCLK, posedge DI[2], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[2]);
        $setuphold (posedge DCLK, posedge DI[3], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[3]);
        $setuphold (posedge DCLK, posedge DI[4], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[4]);
        $setuphold (posedge DCLK, posedge DI[5], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[5]);
        $setuphold (posedge DCLK, posedge DI[6], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[6]);
        $setuphold (posedge DCLK, posedge DI[7], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[7]);
        $setuphold (posedge DCLK, posedge DI[8], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[8]);
        $setuphold (posedge DCLK, posedge DI[9], 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DI_dly[9]);
        $setuphold (posedge DCLK, posedge DWE, 0:0:0, 0:0:0, notifier_do,,,DCLK_dly,DWE_dly);
